...
class LinCFA_fast():
    ...
    def compute_clusters(self):
        ...
        while(len(cols)>0):
            ...
            col_to_aggr = self.find_aggregation(actual_cluster, cols[starting_col:])
            if (col_to_aggr != ''): starting_col = list(cols).index(col_to_aggr) 

            if (col_to_aggr != '') & (len(actual_cluster)<10000):
